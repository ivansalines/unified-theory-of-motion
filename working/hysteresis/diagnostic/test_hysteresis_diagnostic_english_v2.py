#!/usr/bin/env python3
"""Most useful diagnostic version of the hysteresis experiment.

What this version adds:
- transformed coordinates keep rho1, rho2 > 0 and 0 < x < 1 naturally;
- separate settling and measurement windows for each Q step;
- x_mean and x_std are measured only after settling;
- a diagnostic summary dashboard with:
    1) hysteresis loop with capture markers,
    2) residual oscillation amplitude x_std on both branches,
    3) matched-Q branch mismatch: Delta x_mean and Delta x_std,
    4) an upward-branch transition indicator based on the strongest drop of log10(x_std);
- 12 phase spirals (6 upward + 6 downward), color-coded by local time;
- 12 local adiabatic transition panels.

This is meant to be the most informative diagnostic pass rather than the most compact one.
"""
...
SCRIPT_VERSION = "2026-07-31-layout-fix-v2"
...
# Plotting helpers
# ─────────────────────────────────────────────
def infer_grid(n_panels: int) -> tuple[int, int]:
    if n_panels == 12:
        return 3, 4
...
def add_time_colored_curve(ax, x: np.ndarray, y: np.ndarray, t: np.ndarray) -> None:
    points = np.column_stack((x, y)).reshape(-1, 1, 2)
    segments = np.concatenate([points[:-1], points[1:]], axis=1)
    t_mid = 0.5 * (t[:-1] + t[1:])
    lc = LineCollection(segments, array=t_mid, linewidths=1.5)
    ax.add_collection(lc)
    ax.autoscale_view()


def plot_phase_portraits(captures: list[PhaseCapture], output: str | Path) -> Path:
    rows, cols = infer_grid(len(captures))
    ...
    for ax, capture in zip(axes, captures):
        add_time_colored_curve(ax, capture.x_samples, capture.xdot_samples, capture.local_times)
        ax.scatter(capture.x_samples[0], capture.xdot_samples[0], s=32, marker="o", label="start")
        ax.scatter(capture.x_samples[-1], capture.xdot_samples[-1], s=42, marker="x", label="end")
        ax.set_title(
            f"{capture.label}\n"
    ...
    for ax in axes[len(captures):]:
        ax.axis("off")

    # Add a single shared colorbar using the first valid collection.
    first_collection = None
    for ax in axes:
        if ax.collections:
            first_collection = ax.collections[0]
            break
    if first_collection is not None:
        cbar = fig.colorbar(first_collection, ax=axes.tolist(), shrink=0.86)
        cbar.set_label("local time")

    fig.suptitle("Time-colored phase spirals of the third plane: x vs x_dot", fontsize=18)
...
def plot_adiabatic_transition_panels(
    ...
    for ax, capture in zip(axes, captures):
        ...
        ax.axvline(settle_duration, linestyle="--", linewidth=1.2)
        ax.plot(t, x, linewidth=1.6)
        ax.scatter(t[0], x[0], s=26, marker="o", label="start")
        ax.scatter(t[-1], x[-1], s=34, marker="x", label="end")
        ax.set_title(
            f"{capture.label}\n"
            f"{capture.segment.capitalize()} branch | target Q={capture.q_target:.1f}\n"
            f"used Q={capture.q_selected:.3f}",
...
def annotate_capture_points(ax, captures: list[PhaseCapture]) -> None:
    for capture in captures:
        ax.scatter([capture.q_selected], [capture.x_mean], s=30, zorder=5)
        ax.annotate(
            capture.short_label,
            (capture.q_selected, capture.x_mean),
            xytext=(4, 4),
...
def plot_diagnostic_summary(
    Q_up: np.ndarray,
    Q_down: np.ndarray,
    x_mean_up: np.ndarray,
    x_mean_down: np.ndarray,
    x_std_up: np.ndarray,
    x_std_down: np.ndarray,
    captures: list[PhaseCapture],
    transition_q: float,
    ...
    *,
    ...
    output_path = Path(output)
    q_matched = Q_up[:-1].copy()
    x_mean_up_matched = x_mean_up[:-1]
    x_std_up_matched = x_std_up[:-1]
    x_mean_down_matched = x_mean_down[::-1]
    x_std_down_matched = x_std_down[::-1]
    delta_mean = x_mean_down_matched - x_mean_up_matched
    delta_std = x_std_down_matched - x_std_up_matched

    log_std_up = np.log10(np.maximum(x_std_up, 1e-16))
    contraction_indicator = -np.gradient(log_std_up, Q_up)
    ...
    # 1) Hysteresis loop.
    ax1.plot(Q_up, x_mean_up, label="Upward Q sweep", linewidth=2.0)
    ax1.plot(Q_down, x_mean_down, label="Downward Q sweep", linestyle="--", linewidth=2.0)
    ax1.fill_between(Q_up, np.clip(x_mean_up - x_std_up, 0.0, 1.0), np.clip(x_mean_up + x_std_up, 0.0, 1.0), alpha=0.12)
    ax1.fill_between(Q_down, np.clip(x_mean_down - x_std_down, 0.0, 1.0), np.clip(x_mean_down + x_std_down, 0.0, 1.0), alpha=0.12)
    annotate_capture_points(ax1, captures)
    ax1.set_title("Hysteresis loop with residual-oscillation bands")
    ax1.set_xlabel("Parameter Q")
    ax1.set_ylabel("Mean third-plane position (x_mean)")
    ax1.grid(True, alpha=0.3)
    ax1.legend()

    # 2) Residual oscillation amplitude.
    ax2.plot(Q_up, x_std_up, label="Upward branch x_std", linewidth=2.0)
    ax2.plot(Q_down, x_std_down, label="Downward branch x_std", linestyle="--", linewidth=2.0)
    ax2.axvline(transition_q, linestyle=":", linewidth=1.8, label="transition candidate")
    ax2.set_title("Residual oscillation amplitude")
    ax2.set_xlabel("Parameter Q")
    ax2.set_ylabel("x_std during measurement window")
    ax2.grid(True, alpha=0.3)
    ax2.legend()

    # 3) Matched-Q branch mismatch.
    ax3.plot(q_matched, delta_mean, label="Δx_mean = downward - upward", linewidth=2.0)
    ax3.plot(q_matched, delta_std, label="Δx_std = downward - upward", linewidth=2.0)
    ax3.axhline(0.0, linewidth=1.0)
    ax3.set_title("Matched-Q branch mismatch")
    ax3.set_xlabel("Parameter Q")
    ax3.set_ylabel("difference")
    ax3.grid(True, alpha=0.3)
    ax3.legend()

    # 4) Transition indicator.
    ax4.plot(Q_up, contraction_indicator, linewidth=2.0, label="- d/dQ [log10(x_std)]")
    ax4.axvline(transition_q, linestyle=":", linewidth=1.8)
    ax4.scatter([transition_q], [transition_strength], s=40, label="strongest contraction")
    ax4.set_title("Upward-branch transition indicator")
    ax4.set_xlabel("Parameter Q")
    ax4.set_ylabel("contraction indicator")
    ax4.grid(True, alpha=0.3)
    ax4.legend()

    fig.suptitle(
        f"Most useful diagnostic summary (n={n:g})\n"
        f"{settle_periods:g} settling periods + {measure_periods:g} measurement periods per Q",
        fontsize=18,
    )
    fig.savefig(output_path, dpi=180)
    plt.close(fig)
    print(f"Diagnostic summary saved to: {output_path.resolve()}")
    return output_path
...
def run_hysteresis(
    *,
    ...
    rtol: float = 1e-8,
    atol: float = 1e-10,
    summary_output: str | Path = "diagnostic_summary.png",
    spirals_output: str | Path = "diagnostic_phase_spirals.png",
    transition_output: str | Path = "diagnostic_adiabatic_transition_panels.png",
) -> tuple[Path, Path, Path]:
    if Q_steps < 2:
        raise ValueError("Q_steps must be at least 2.")
    if spirals_per_branch < 2:
        raise ValueError("spirals_per_branch must be at least 2.")
    ...
    initial_physical_state = np.array(
        [rho0, 0.0, 0.0, +n * omega0, rho0, 0.0, delta0, -n * omega0, x0, 0.0, 0.0],
        dtype=float,
    )
    ...
    target_levels = np.linspace(Q_min, Q_max, spirals_per_branch)
    targets: list[PhaseTarget] = []
    idx = 1
    for q in target_levels:
        targets.append(PhaseTarget(f"Spiral {idx}: upward sweep", f"S{idx}", "upward", float(q)))
        idx += 1
    for q in target_levels[::-1]:
        targets.append(PhaseTarget(f"Spiral {idx}: downward sweep", f"S{idx}", "downward", float(q)))
        idx += 1
    ...
    if gamma_rho == 0.0:
        print("WARNING: without radial damping, the rho modes do not truly settle and long sweeps may grow without bound.")
    print(f"Damped x period       = {period_x:.6f}")
    print(f"Initial Delta period  = {period_delta:.6f}")
    print(f"Reference period      = {period_ref:.6f}")
    print(f"Per Q: {settle_periods:g} reference periods for settling + {measure_periods:g} reference periods for measurement")

    total_duration = settle_duration + measure_duration
    total_sample_count = max(64, int(np.ceil((settle_periods + measure_periods) * samples_per_period)) + 1)
    ...
    for index, Q_current in enumerate(Q_path, start=1):
        ...
        x_std = float(np.std(x_measure))
        x_mean_history.append(float(x_mean))
        x_std_history.append(x_std)
        ...
        if index == 1 or index == total or index % max(1, total // 10) == 0:
            ...
            print(
                f"[{index:>3}/{total}] Q={Q_current:8.3f}  x_mean={x_mean:.7f}  x_std={x_std:.3e}  rho=({physical[0]:.4f}, {physical[4]:.4f})"
            )

    x_mean_array = np.asarray(x_mean_history)
    x_std_array = np.asarray(x_std_history)
    x_mean_up = x_mean_array[:Q_steps]
    x_mean_down = x_mean_array[Q_steps:]
    x_std_up = x_std_array[:Q_steps]
    x_std_down = x_std_array[Q_steps:]

    captures = [capture for capture in best_capture if capture is not None]
    if len(captures) != len(targets):
        raise RuntimeError("Could not capture all requested spirals.")

    log_std_up = np.log10(np.maximum(x_std_up, 1e-16))
    contraction_indicator = -np.gradient(log_std_up, Q_up)
    transition_index = int(np.argmax(contraction_indicator))
    transition_q = float(Q_up[transition_index])
    transition_strength = float(contraction_indicator[transition_index])

    print(f"Transition candidate on upward branch: Q ≈ {transition_q:.6f}")
    print(f"Transition indicator strength          = {transition_strength:.6e}")
    print(f"Maximum upward x_std                  = {x_std_up.max():.6e} at Q={Q_up[np.argmax(x_std_up)]:.6f}")
    matched_delta_mean = x_mean_down[::-1] - x_mean_up[:-1]
    matched_delta_std = x_std_down[::-1] - x_std_up[:-1]
    print(f"Maximum matched |Δx_mean|             = {np.max(np.abs(matched_delta_mean)):.6e}")
    print(f"Maximum matched |Δx_std|              = {np.max(np.abs(matched_delta_std)):.6e}")

    summary_path = plot_diagnostic_summary(
        Q_up,
        Q_down,
        x_mean_up,
        x_mean_down,
    ...
    spirals_path = plot_phase_portraits(captures, spirals_output)
    transition_path = plot_adiabatic_transition_panels(captures, settle_duration, transition_output)

    return summary_path, spirals_path, transition_path
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Most useful diagnostic hysteresis run with 12 spirals and transition diagnostics."
    )
    ...
    parser.add_argument("--spirals-per-branch", type=int, default=6)
    ...
    parser.add_argument("--summary-output", default="diagnostic_summary.png")
    parser.add_argument("--spirals-output", default="diagnostic_phase_spirals.png")
    parser.add_argument("--transition-output", default="diagnostic_adiabatic_transition_panels.png")
...
def main() -> None:
    ...
    run_hysteresis(
        ...
        spirals_per_branch=args.spirals_per_branch,
