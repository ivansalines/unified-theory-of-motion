...
- colore continuo sul winding (o sullo E, a scelta) invece di due soli
...
REQUIRED_COLUMNS = {"E", "Numberline_Q", "Type"}


def parse_args():
    p = argparse.ArgumentParser(description="Plot 3D dei dati di winding (E / Numberline_Q / Winding).")
    ...
    p.add_argument("--color-by", choices=["winding", "E"], default="winding",
    ...
    p.add_argument("--out", default="windings_3d_E.html",
                    help="File di output (HTML per plotly, PNG per matplotlib se --no-show)")
    p.add_argument("--stiff-size", type=float, default=1.5, help="Dimensione punti Stiffness (default: 1.5)")
    p.add_argument("--stiff-opacity", type=float, default=0.55, help="Opacita' punti Stiffness (default: 0.55)")
    p.add_argument("--conv-size", type=float, default=2.5, help="Dimensione punti Converged (default: 2.5)")
    p.add_argument("--conv-opacity", type=float, default=0.5, help="Opacita' punti Converged (default: 0.5)")
...
def plot_plotly(data, color_by, out_path, show, stiff_size, stiff_opacity, conv_size, conv_opacity):
    ...
    color_column = {"winding": "Winding", "E": "E"}[color_by]

    fig.add_trace(go.Scatter3d(
        x=stiff["Winding"], y=stiff["Numberline_Q"], z=stiff["E"],
        mode="markers",
        name="Stiffness",
        marker=dict(
            size=stiff_size,
            ...
            opacity=stiff_opacity,
            line=dict(width=0),  # niente bordo: e' il bordo a "sporcare" la nuvola con tanti punti
        ),
        hovertemplate="Winding=%{x}<br>Numberline_Q=%{y}<br>E=%{z}<extra>Stiffness</extra>",
    ))

    fig.add_trace(go.Scatter3d(
        x=conv["Winding"], y=conv["Numberline_Q"], z=conv["E"],
        mode="markers",
        name="Converged",
        marker=dict(
            size=conv_size,
            color="red",
            symbol="diamond",
            opacity=conv_opacity,
            line=dict(width=0),
        ),
        hovertemplate="Winding=%{x}<br>Numberline_Q=%{y}<br>E=%{z}<extra>Converged</extra>",
    ))
...
def plot_matplotlib(data, elev, azim, out_path, show):
    ...
    sc = ax.scatter(
        stiff["Winding"], stiff["Numberline_Q"], stiff["E"],
        c=stiff["Winding"], cmap="viridis", s=6, alpha=0.7, label="Stiffness"
    )
    ax.scatter(
        conv["Winding"], conv["Numberline_Q"], conv["E"],
...
def main():
    ...
    data = data.sort_values(by="E", ascending=(args.sort_mode == "asc"))

    if args.engine == "plotly":
        try:
            out_path = args.out if args.out.endswith(".html") else args.out + ".html"
            plot_plotly(
                data, args.color_by, out_path, show=not args.no_show,
                stiff_size=args.stiff_size, stiff_opacity=args.stiff_opacity,
                conv_size=args.conv_size, conv_opacity=args.conv_opacity,
            )
        except ImportError:
