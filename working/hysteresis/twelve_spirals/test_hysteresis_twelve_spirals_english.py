#!/usr/bin/env python3
"""Hysteresis test with adiabatic sweep, twelve phase spirals, and transition views.
...
- the script saves:
  ...
  2) twelve phase portraits x vs x_dot,
  3) twelve local time traces showing the adiabatic transition.

The twelve phase captures are chosen at six evenly spaced Q levels on the
upward branch and the same six Q levels on the downward branch.
"""
...
def plot_phase_portraits(captures: list[PhaseCapture], output: str | Path) -> Path:
    ...
    for ax, capture in zip(axes, captures):
        ...
        for i in range(len(xs) - 1):
            ...
            ax.plot(xs[i:i+2], xds[i:i+2], linewidth=1.1, alpha=alpha)

        ax.scatter(xs[0], xds[0], s=34, marker="o", label="start")
        ax.scatter(xs[-1], xds[-1], s=44, marker="x", label="end")
        ax.set_title(
            f"{capture.label}\n"
    ...
    fig.suptitle("Twelve phase spirals of the third plane: x vs x_dot", fontsize=18)
...
def plot_adiabatic_transition_panels(
    ...
    rows, cols = infer_grid(len(captures))
    output_path = Path(output)
    fig, axes = plt.subplots(rows, cols, figsize=(4.8 * cols, 4.2 * rows), sharey=False)
    axes = np.atleast_1d(axes).ravel()
    ...
    fig.suptitle(
        "Adiabatic transition snapshots: local response at twelve selected Q steps",
        fontsize=18,
    )
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    fig.savefig(output_path, dpi=180)
...
def annotate_capture_points(ax, captures: list[PhaseCapture]) -> None:
    for capture in captures:
        ax.scatter([capture.q_selected], [capture.x_mean], s=34, zorder=5)
        ax.annotate(
            capture.short_label,
...
def run_hysteresis(
    *,
    ...
    spirals_output: str | Path = "twelve_phase_spirals.png",
    transition_output: str | Path = "twelve_adiabatic_transition_panels.png",
    ...
    if gamma_rho == 0.0:
        print(
            "WARNING: without radial damping, the rho modes do not truly settle and long sweeps may grow without bound."
        )
    ...
    for index, Q_current in enumerate(Q_path, start=1):
        ...
        if index == 1 or index == total or index % max(1, total // 10) == 0:
            ...
            print(
                f"[{index:>3}/{total}] Q={Q_current:8.3f}  x_mean={x_mean:.7f}  x_std={x_std_history[-1]:.3e}  rho=({physical[0]:.4f}, {physical[4]:.4f})"
            )
    ...
    ax.plot(Q_down, x_mean_array[downward_slice], label="Downward Q sweep", linestyle="--", linewidth=2.0)
    ax.fill_between(
        Q_up,
    ...
    annotate_capture_points(ax, captures)
    ax.set_title(
        f"Topological hysteresis test (n={n:g})\nmean over {measure_periods:g} reference periods after {settle_periods:g} settling periods"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Hysteresis test with twelve phase spirals and adiabatic transition views."
    )
    ...
    parser.add_argument("--spirals-output", default="twelve_phase_spirals.png")
    parser.add_argument(
        "--transition-output",
        default="twelve_adiabatic_transition_panels.png",
