...
# with a Lagrange multiplier λ(t) enforcing Q exactly (up to discretization error).
#
# Energy:
...
def relax_constrained_aggressive(Q_target, rho0=None,
                                 ...
                                 tol_force=5e-8):
    """
    Constrained gradient flow with adaptive time step (backtracking line search).
    ...
    for k in range(1, max_steps + 1):
        ...
        # residual norm (for diagnostics and stopping)
        F2 = integrate_axisym(F**2)
        rho2 = integrate_axisym(rho**2)
        fnorm = np.sqrt(F2 / (rho2 + 1e-30))

        # attempt an explicit step with current dt
        attempt_ok = False
        n_backtrack = 0

        while not attempt_ok:
            if dt < dt_min:
                print("  !! dt fell below dt_min: stiffness-limited termination.")
                return rho
            ...
            if E_trial <= E_prev:  # energia non cresce: accetto
                ...
                # energy increased => backtracking
                dt *= 0.5
                n_backtrack += 1
                if n_backtrack > 20:
                    print("  !! Too many backtracking attempts: likely stiffness/instability.")
                    return rho
        ...
        # mild dt growth when steps are accepted without backtracking
...
# ============================================================
# Stability / time-horizon robustness test
# ============================================================
...
def stability_test(rho_star, Q_target, seed=0):
    ...
    # Perturbation: multiplicative low-amplitude noise
    ...
    # thresholds for collapse/runaway heuristics (tunable)
