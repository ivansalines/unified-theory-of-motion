...
- colore continuo sul winding (o sullo ||F||, a scelta) invece di due soli
...
REQUIRED_COLUMNS = {"||F||", "Numberline_Q", "Type"}


def parse_args():
    p = argparse.ArgumentParser(description="Plot 3D dei dati di winding (||F|| / Numberline_Q / Winding).")
    ...
    p.add_argument("--color-by", choices=["winding", "||F||"], default="winding",
    ...
    p.add_argument("--out", default="windings_3d_F.html",
...
def plot_plotly(data, color_by, out_path, show, stiff_size, stiff_opacity, conv_size, conv_opacity):
    ...
    color_column = {"winding": "Winding", "||F||": "||F||"}[color_by]

    fig.add_trace(go.Scatter3d(
        x=stiff["Winding"], y=stiff["Numberline_Q"], z=stiff["||F||"],
        mode="markers",
        name="Stiffness",
        marker=dict(
            size=stiff_size,
        ...
        hovertemplate="Winding=%{x}<br>Numberline_Q=%{y}<br>||F||=%{z}<extra>Stiffness</extra>",
    ))

    fig.add_trace(go.Scatter3d(
        x=conv["Winding"], y=conv["Numberline_Q"], z=conv["||F||"],
        mode="markers",
        name="Converged",
        marker=dict(
            size=conv_size,
        ...
        hovertemplate="Winding=%{x}<br>Numberline_Q=%{y}<br>||F||=%{z}<extra>Converged</extra>",
    ))
...
def plot_matplotlib(data, elev, azim, out_path, show):
    ...
    sc = ax.scatter(
        stiff["Winding"], stiff["Numberline_Q"], stiff["||F||"],
        c=stiff["Winding"], cmap="viridis", s=6, alpha=0.7, label="Stiffness"
    )
    ax.scatter(
        conv["Winding"], conv["Numberline_Q"], conv["||F||"],
...
def main():
    ...
    data = data.sort_values(by="||F||", ascending=(args.sort_mode == "asc"))
